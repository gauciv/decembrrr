...
function StudentDashboard() {
  ...
  const dailyAmount = classData?.daily_amount ?? 10;
  const daysCovered = Math.max(0, Math.floor(profile.balance / dailyAmount));
      ...
      >
        ...
        <CardContent>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {classData?.name} · ₱{dailyAmount}/{classData?.collection_frequency === "weekly" ? "week" : "day"}
          </p>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card>
          <CardContent className="pt-6">
            <p className="text-2xl font-bold">{daysCovered}</p>
            <p className="text-xs text-muted-foreground">
